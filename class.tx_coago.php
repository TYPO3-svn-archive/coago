...
class tx_coago {
	...
	function getCobjHash() {
		...
		if( is_array($this->TSObjectConf['cache.']['hash.']['special.']) ){
			...
			if($pidList) {
				$additionalWhere = $this->TSObjectConf['cache.']['hash.']['special.']['unique.']['pidList.']['additionalWhere'];
				$rows = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows ('uid', 'pages', 'pid IN('. trim($pidList) . ') AND doktype <> 254 AND hidden=0 AND deleted=0 ' . $additionalWhere, $groupBy='', $orderBy='', $limit='');
				if($rows) {
					foreach($rows as $row) {
						$uidList .= ',' . $row['uid'];
					}
	...
	function getCoagoAjaxScript() {
		...
		if($this->TSObjectConf['cache.']['hash.']['special.']['feuser']) {
		  ...
		  '{$siteUrl}index.php?eID=coagoFtu&cacheHash={$this->cacheHash}&typeNum={$this->typeNum}&cachePeriod={$this->cachePeriod}',
		  {$this->onLoading},
		  {$this->refresh});";

		} else {
			$script = "coago({$this->counter},
		  '{$siteUrl}{$this->relativePath}{$this->filename}{$ftu}.{$this->cacheFileExtension}',
		  '{$siteUrl}index.php?id={$GLOBALS['TSFE']->id}&type={$this->typeNum}&cacheHash={$this->cacheHash}{$ftuHash}',
		  {$this->cachePeriod},
		  {$this->onLoading},
		  {$this->refresh});";
		}
